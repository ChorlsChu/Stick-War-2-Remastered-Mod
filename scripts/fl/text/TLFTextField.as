   ...
   public class TLFTextField extends Sprite
   {
      ...
      tlf_internal function repaint(e:Event = null) : void
      {
         ...
         try
         {
            ...
            if(this.TCMUsesTextStringAndFormat || Boolean(this._containerManager.textFlow))
            {
               if(Boolean(this._invalidState & (INVALID_AUTO_SIZE | INVALID_WORD_WRAP)))
               {
                  ...
                  if(Boolean(this._invalidState & INVALID_WORD_WRAP))
                  {
                  ...
                  else if(Boolean(this._invalidState & INVALID_AUTO_SIZE) && Boolean(this._prevAutoSize != null) && this._prevAutoSize != this._autoSize)
                  {
                     if(!this.objInit)
                     {
                        if(this._wordWrap)
               ...
               if(Boolean(this._invalidState & INVALID_TEXT) && this._invalidTextLayoutFormat != null)
               {
            ...
            if(Boolean(this._invalidState & (INVALID_TEXT | INVALID_DIMENSIONS | INVALID_WORD_WRAP | INVALID_AUTO_SIZE)))
            {
               this._containerManager.update();
            }
            if(Boolean(this._invalidState & (INVALID_TEXT | INVALID_DIMENSIONS | INVALID_BORDER | INVALID_WORD_WRAP | INVALID_AUTO_SIZE)))
            {
      ...
      public function get styleSheet() : StyleSheet
      {
         ...
         if(!this.TCMUsesTextStringAndFormat && Boolean(this.textFlow))
         {
      ...
      {
         if(!this.TCMUsesTextStringAndFormat && this.textFlow.cffHinting == CFFHinting.NONE || Boolean(this.TCMUsesTextStringAndFormat && this.hostFormat) && Boolean(this.hostFormat.cffHinting == CFFHinting.NONE))
         {
      ...
      tlf_internal function composeComplete(event:CompositionCompleteEvent) : void
      {
         ...
         if(Boolean(event) && Boolean(!this.TCMUsesTextStringAndFormat) && event.target as TextFlow != this.textFlow)
         {
            return;
         }
      ...
      {
         if(this._displayAsPassword || Boolean(!this.TCMUsesTextStringAndFormat && this.textFlow) && Boolean(this.textFlow.generation == this._generationID))
         {
            return this._text;
         }
         if(this._text == "" || this.TCMUsesTextStringAndFormat || Boolean(this.textFlow) && Boolean(this.textFlow.generation != this._generationID))
         {
      ...
      {
         var master:TLFTextField = this.firstField;
         return Boolean(master) ? master._alwaysShowSelection : this._alwaysShowSelection;
      }
      ...
      public function set defaultTextFormat(value:TextFormat) : void
      {
         ...
         if(Boolean(!this.TCMUsesTextStringAndFormat) && Boolean(this.textFlow) && this.textFlow.formatResolver != null)
         {
            throw new Error("This method cannot be used on a text field with a style sheet.");
         }
      ...
      tlf_internal function textFlow_flowOperationBeginHandler(event:FlowOperationEvent) : void
      {
         ...
         if(op is SplitParagraphOperation)
         {
            ...
            numToDel = originalSelectionState == null ? 0 : int(originalSelectionState.absoluteEnd - originalSelectionState.absoluteStart);
            if(this.maxChars != 0)
            {
         ...
         if(op is InsertTextOperation)
         {
            ...
            delLen = delSelOp == null ? 0 : int(delSelOp.absoluteEnd - delSelOp.absoluteStart);
            if(this.maxChars != 0)
            {
      ...
      public function getCharIndexAtPoint(x:Number, y:Number) : int
      {
         ...
         if(Boolean(tf))
         {
            ...
            return rect.containsPoint(newPoint) ? idx : (idx > 0 ? int(idx - 1) : idx);
         }
         return -1;
      }
...
TextFlow.defaultConfiguration.manageTabKey = false;
