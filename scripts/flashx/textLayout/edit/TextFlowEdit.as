package flashx.textLayout.edit
{
   ...
   public class TextFlowEdit
   {
      ...
      {
         while(Boolean(element) && !element.canOwnFlowElement(prospectiveChild))
         {
      ...
      public static function insertTextScrap(textFlow:TextFlow, absoluteStart:int, textScrap:TextScrap, applyFormat:Boolean) : int
      {
         ...
         while(Boolean(scrapLeaf))
         {
            ...
            destinationParagraph = destinationLeaf.getParagraph();
            ...
            else
            {
               ...
               while(Boolean(scrapElement && scrapElement.parent) && (Boolean(!scrapElement.parent.format || scrapElement.parent.format.getStyle(ConverterBase.MERGE_TO_NEXT_ON_PASTE) === undefined)) && !(scrapElement.parent is TextFlow))
               {
            ...
            insertPosition = Boolean(destinationLeaf) ? destinationLeaf.getAbsoluteStart() : int(textFlow.textLength - 1);
      ...
      public static function makeLink(theFlow:TextFlow, startPos:int, endPos:int, urlString:String, target:String) : Boolean
      {
         ...
         while(Boolean(curPara))
         {
            paraEnd = curPara.getAbsoluteStart() + curPara.textLength;
            curEndPos = Math.min(paraEnd,endPos);
            linkEndPos = curEndPos == paraEnd ? int(curEndPos - 1) : curEndPos;
      ...
      tlf_internal static function insertNewSPBlock(theFlow:TextFlow, startPos:int, endPos:int, newSPB:SubParagraphGroupElementBase, spgClass:Class) : Boolean
      {
         ...
         if(Boolean(curFBE.parent) && Boolean(curFBE.parent is spgClass) && !(parentStart == curFBEStart && parentStart + curFBE.parent.textLength == curFBEStart + curFBE.textLength))
         {
            return false;
         }
      ...
      tlf_internal static function findAndRemoveFlowGroupElement(theFlow:TextFlow, startPos:int, endPos:int, fbeClass:Class) : Boolean
      {
         ...
         while(curPos < endPos)
         {
            ...
            while(Boolean(containerFBE.parent && containerFBE.parent.getAbsoluteStart() == containerFBE.getAbsoluteStart()) && Boolean(!(containerFBE.parent is ParagraphElement)) && !(containerFBE is ParagraphElement))
            {
