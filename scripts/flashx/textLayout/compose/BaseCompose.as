package flashx.textLayout.compose
{
   ...
   public class BaseCompose
   {
      ...
      private function advanceToComposeStartPosition() : Boolean
      {
         ...
         if(Boolean(curLine.controller.numFloats))
         {
            if(this._measuring)
            {
               return false;
         ...
         if(!this._paragraphContainsVisibleLines)
         {
            ...
            if(okToRelease)
            {
               for(textLine = textBlock.lastLine; Boolean(textLine); )
               {
                  ...
                  if(Boolean(this._textFlow.backgroundManager))
                  {
      ...
      protected function getFirstIndentCharPos(paragraph:ParagraphElement) : int
      {
         ...
         while(Boolean(leaf) && Boolean(leaf is InlineGraphicElement) && InlineGraphicElement(leaf).effectiveFloat != Float.NONE)
         {
            pos += leaf.textLength;
      ...
      protected function composeInlineGraphicElement(inlineGraphic:InlineGraphicElement, textLine:TextLine) : Boolean
      {
         ...
         this._curParcel.controller.addFloatAt(this._curParaStart + inlineGraphic.getElementRelativeStart(this._curParaElement),inlineGraphic.graphic,Float.NONE,marginAndPaddingX,marginAndPaddingY,Boolean(fteInline) ? fteInline.alpha : 1,Boolean(fteInline) ? fteInline.transform.matrix : null,this._parcelList.totalDepth,0,this._curParcel.columnIndex,textLine);
         return true;
      }
      ...
      protected function composeFloat(elem:InlineGraphicElement, afterLine:Boolean) : Boolean
      {
         ...
         if(Boolean(this._curLine) && Boolean(this._curParaElement != this._curLine.paragraph) && !this._atColumnStart)
         {
      ...
      private function getRightSideGap(curLine:TextFlowLine, aligned:Boolean) : Number
      {
         ...
         do
         {
            rightSideGap += this._blockProgression == BlockProgression.TB ? elem.getEffectivePaddingRight() : elem.getEffectivePaddingBottom();
            elem = elem.parent;
         }
      ...
      private function getLeftSideGap(curLine:TextFlowLine) : Number
      {
         ...
         do
         {
            leftSideGap += this._blockProgression == BlockProgression.TB ? elem.getEffectivePaddingLeft() : elem.getEffectivePaddingTop();
            elem = elem.parent;
         }
         while(!(elem is TextFlow));
         return leftSideGap;
         ...
         if(Boolean(alignData))
         {
            ...
            if(this._blockProgression == BlockProgression.TB)
            {
               if(!this._measuring)
               {
                  ...
                  if(Boolean(numberLine) && TextFlowLine.getNumberLineListStylePosition(numberLine) == ListStylePosition.OUTSIDE)
                  {
      ...
      protected function fitLineToParcel(textLine:TextLine, isNewLine:Boolean, numberLine:TextLine) : Boolean
      {
         ...
         for(var spaceBefore:Number = Math.max(this._curLine.spaceBefore,this._paragraphSpaceCarried); true; )
         {
            ...
            if(Boolean(this._pushInFloats) && Boolean(this._parcelList.currentParcel.fitAny) && this._pushInFloats.length > 0)
            {
               break;
            }
            while(true)
      ...
      private function clearControllers(oldController:ContainerController, newController:ContainerController) : void
      {
         ...
         var lastToClear:int = Boolean(newController) ? this._flowComposer.getControllerIndex(newController) : int(this._flowComposer.numControllers - 1);
