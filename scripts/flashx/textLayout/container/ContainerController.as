   ...
   public class ContainerController implements IInteractionEventHandler, ITextLayoutFormat, ISandboxSupport
   {
      ...
      public function setCompositionSize(w:Number, h:Number) : void
      {
         ...
         if(widthChanged || heightChanged)
         {
            ...
            if(this.effectiveBlockProgression == BlockProgression.TB ? widthChanged : heightChanged)
            {
      ...
      tlf_internal function setRootElement(value:ContainerFormattedElement) : void
      {
         if(this._rootElement != value)
         {
            ...
            if(Boolean(this._container) && Configuration.playerEnablesSpicyFeatures)
            {
      ...
      private function gatherVisibleLines(wmode:String, createShape:Boolean) : void
      {
         ...
         for(var lineIndex:int = firstLine; lineIndex <= lastLine; lineIndex++)
         {
            ...
            {
               textLine = this.isLineVisible(wmode,scrollAdjustXTW,scrollAdjustYTW,scrollAdjustWidthTW,scrollAdjustHeightTW,curLine,null);
               if(Boolean(textLine))
               {
                  if(createShape)
                  {
      ...
      tlf_internal function checkScrollBounds() : void
      {
         ...
         if(Boolean(this.textFlow) && Boolean(this._container) && !this._minListenersAttached)
         {
      ...
      private function get containerScrollRectLeft() : Number
      {
         ...
         {
            rslt = this.effectiveBlockProgression == BlockProgression.RL ? this.horizontalScrollPosition - this.compositionWidth : this.horizontalScrollPosition;
         }
         return rslt;
      }
      ...
      tlf_internal function scrollTimerHandler(event:Event) : void
      {
         ...
         else if(!event)
         {
            this._scrollTimer.stop();
            this._scrollTimer.removeEventListener(TimerEvent.TIMER,this.scrollTimerHandler);
            if(Boolean(this.getContainerRoot()))
            {
      ...
      {
         if(Boolean(this.interactionManager) && Boolean(event) && !event.isDefaultPrevented())
         {
      ...
      tlf_internal function addSelectionShapes(selFormat:SelectionFormat, selectionAbsoluteStart:int, selectionAbsoluteEnd:int) : void
      {
         ...
         if(selectionAbsoluteStart != selectionAbsoluteEnd)
         {
            ...
            endLine = selectionAbsoluteStart == selectionAbsoluteEnd ? begLine : this.flowComposer.findLineIndexAtPosition(selectionAbsoluteEnd);
            ...
            for(idx = begLine; idx <= endLine; idx++)
            {
               ...
               line.hiliteBlockSelection(selObj,selFormat,this._container,selectionAbsoluteStart < line.absoluteStart ? line.absoluteStart : selectionAbsoluteStart,selectionAbsoluteEnd > line.absoluteStart + line.textLength ? int(line.absoluteStart + line.textLength) : selectionAbsoluteEnd,prevLine,nextLine);
               temp = line;
               line = nextLine;
               prevLine = temp;
            }
      ...
      {
         return this.getEffectivePaddingLeft() + (Boolean(this._rootElement) ? this._rootElement.getEffectivePaddingLeft() : 0);
      }
      
      tlf_internal function getTotalPaddingRight() : Number
      {
         return this.getEffectivePaddingRight() + (Boolean(this._rootElement) ? this._rootElement.getEffectivePaddingRight() : 0);
      }
      
      tlf_internal function getTotalPaddingTop() : Number
      {
         return this.getEffectivePaddingTop() + (Boolean(this._rootElement) ? this._rootElement.getEffectivePaddingTop() : 0);
      }
      
      tlf_internal function getTotalPaddingBottom() : Number
      {
         return this.getEffectivePaddingBottom() + (Boolean(this._rootElement) ? this._rootElement.getEffectivePaddingBottom() : 0);
      }
      ...
      tlf_internal function updateCompositionShapes() : void
      {
         ...
         if(Boolean(this._floatsInContainer) && Boolean(this._floatsInContainer.length > 0) || Boolean(this._composedFloats) && Boolean(this._composedFloats.length > 0))
         {
         ...
         this._mouseEventManager.updateHitTests(this.effectiveBlockProgression == BlockProgression.RL && this._hasScrollRect ? this._contentWidth : 0,scratchRectangle,tf,Boolean(firstVisibleLine) ? firstVisibleLine.absoluteStart : this._absoluteStart,Boolean(lastVisibleLine) ? int(lastVisibleLine.absoluteStart + lastVisibleLine.textLength - 1) : this._absoluteStart,needsCtrlKey);
         this._updateStart = this._rootElement.textLength;
         if(this._measureWidth || this._measureHeight)
         {
            this.attachTransparentBackgroundForHit(false);
         }
         if(Boolean(tf.backgroundManager))
         {
      ...
      tlf_internal function updateGraphics(updateStart:int) : void
      {
         ...
         var lastVisiblePosition:int = Boolean(lastLine) ? int(lastLine.absoluteStart + lastLine.textLength) : int(this.absoluteStart + this.textLength);
         var followingLine:TextFlowLine = this.flowComposer.findLineAtPosition(lastVisiblePosition);
         var lastPossibleFloatPosition:int = Boolean(followingLine) ? int(followingLine.absoluteStart + followingLine.textLength) : int(this.absoluteStart + this.textLength);
         ...
         while(floatIndex < numFloats)
         {
            ...
            else
            {
               ...
               if(Boolean(containerListIndex < numContainerList && floatInfo.parent == this._container && inlineHolder) && Boolean(inlineHolder.parent == this._container) && float == this._floatsInContainer[containerListIndex])
               {
      ...
      protected function updateVisibleRectangle() : void
      {
         ...
         else
         {
            ...
            xOrigin = this.effectiveBlockProgression == BlockProgression.RL ? -width : 0;
      ...
      {
         if(Boolean(TextLayoutFormat.description[styleProp]))
         {
      ...
      tlf_internal function isLineVisible(wmode:String, scrollXTW:int, scrollYTW:int, scrollWidthTW:int, scrollHeightTW:int, textFlowLine:TextFlowLine, textLine:TextLine) : TextLine
      {
         ...
         if((wmode == BlockProgression.TB ? Boolean(this._measureHeight) : Boolean(this._measureWidth)) || textFlowLine.isLineVisible(wmode,scrollXTW,scrollYTW,scrollWidthTW,scrollHeightTW))
         {
