package flashx.textLayout.container
{
   ...
   public class TextContainerManager extends EventDispatcher implements ISWFContext, IInteractionEventHandler, ISandboxSupport
   {
         ...
         if(this._textDamaged || this._lastSeparator != separator)
         {
            ...
            if(firstLeaf != null)
            {
               ...
               while(Boolean(para))
               {
                  ...
                  this._text += Boolean(nextPara) ? separator : "";
                  para = nextPara;
               }
            }
            this._textDamaged = false;
      ...
      public function setTextFlow(textFlow:TextFlow) : void
      {
         ...
         if(Boolean(textFlow.flowComposer) && Boolean(textFlow.flowComposer.numControllers > 0) && textFlow.flowComposer.getControllerAt(0) is TMContainerController)
         {
         ...
         this._composeState = Boolean(textFlow.interactionManager) || textFlow.mustUseComposer() ? COMPOSE_COMPOSER : COMPOSE_FACTORY;
      ...
      {
         return this._composeState == COMPOSE_COMPOSER ? Number(this.getController().horizontalScrollPosition) : 0;
      }
      ...
      {
         return this._composeState == COMPOSE_COMPOSER ? Number(this.getController().verticalScrollPosition) : 0;
      }
      ...
      public function callInContext(fn:Function, thisArg:Object, argsArray:Array, returns:Boolean = true) : *
      {
         ...
         if(Boolean(textBlock) && _expectedFactoryCompose == TextLineFactoryBase._factoryComposer)
         {
      ...
      private function invalidateInteractionManager(alwaysRecreateManager:Boolean = false) : void
      {
         ...
         if(this._composeState == COMPOSE_COMPOSER)
         {
            ...
            activePos = Boolean(interactionManager) ? interactionManager.activePosition : -1;
            anchorPos = Boolean(interactionManager) ? interactionManager.anchorPosition : -1;
            if(this._editingMode == EditingMode.READ_ONLY)
            {
      ...
      {
         if(Boolean(this._composedLines))
         {
      ...
      private function addActivationEventListeners() : void
      {
         ...
         if(this._composeState == COMPOSE_FACTORY)
         {
            ...
            {
               newState = this._handlersState == HANDLERS_NOTADDED ? HANDLERS_CREATION : HANDLERS_ACTIVE;
            }
         }
      ...
      private function get effectiveBlockProgression() : String
      {
         ...
         return Boolean(this._hostFormat) && Boolean(this._hostFormat.blockProgression) && this._hostFormat.blockProgression != FormatValue.INHERIT ? this._hostFormat.blockProgression : BlockProgression.TB;
      }
      ...
      public function drawBackgroundAndSetScrollRect(scrollX:Number, scrollY:Number) : Boolean
      {
         ...
         {
            contentLeft = this._composeState == COMPOSE_FACTORY ? this._contentLeft : controller.contentLeft;
         ...
         {
            contentTop = this._composeState == COMPOSE_FACTORY ? this._contentTop : controller.contentTop;
