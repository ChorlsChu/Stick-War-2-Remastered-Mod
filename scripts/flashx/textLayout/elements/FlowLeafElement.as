package flashx.textLayout.elements
{
   ...
   public class FlowLeafElement extends FlowElement
   {
      ...
      {
         return Boolean(this._eventMirror) && this._eventMirror._listenerCount != 0;
      }
      ...
      override tlf_internal function setParentAndRelativeStart(newParent:FlowGroupElement, newStart:int) : void
      {
         ...
         if(Boolean(this._blockElement) && Boolean(parent) && parent.hasBlockElement())
         {
      ...
      {
         return Boolean(this._text) ? this._text.charAt(relativePosition) : "";
      }
      ...
      {
         var tf:TextFlow = getTextFlow();
         return computeElementFormatHelper(_computedFormat,getParagraph(),Boolean(tf) && Boolean(tf.flowComposer) ? tf.flowComposer.swfContext : null);
      }
      ...
      tlf_internal function updateAdornments(tLine:TextLine, blockProgression:String) : int
      {
         ...
         var i:int = 0;
         if(Boolean(_computedFormat.textDecoration == TextDecoration.UNDERLINE) || Boolean(_computedFormat.lineThrough) || _computedFormat.backgroundAlpha > 0 && _computedFormat.backgroundColor != BackgroundColor.TRANSPARENT)
         {
      ...
      private function updateAdornmentsOnBounds(tLine:TextLine, blockProgression:String, spanBounds:Rectangle) : void
      {
         ...
         if(blockProgression != BlockProgression.RL)
         {
            ...
            if(Boolean(_computedFormat.lineThrough))
            {
