      ...
      tlf_internal static function computeEndIdx(targetFlowLine:TextFlowLine, curTextFlowLine:TextFlowLine, blockProgression:String, isRTLDirection:Boolean, globalPoint:Point) : int
      {
         ...
         if(atomIndex == -1)
         {
            if(blockProgression != BlockProgression.RL)
            {
               if(!bidiRightToLeft)
               {
                  endIdx = globalPoint.x <= targetTextLine.x ? targetFlowLine.absoluteStart : int(targetFlowLine.absoluteStart + targetFlowLine.textLength - 1);
               }
               else
               {
                  endIdx = globalPoint.x <= targetTextLine.x ? int(targetFlowLine.absoluteStart + targetFlowLine.textLength - 1) : targetFlowLine.absoluteStart;
               }
            }
            else if(!bidiRightToLeft)
            {
               endIdx = globalPoint.y <= targetTextLine.y ? targetFlowLine.absoluteStart : int(targetFlowLine.absoluteStart + targetFlowLine.textLength - 1);
            }
            else
            {
               endIdx = globalPoint.y <= targetTextLine.y ? int(targetFlowLine.absoluteStart + targetFlowLine.textLength - 1) : targetFlowLine.absoluteStart;
            }
         }
         else
         {
