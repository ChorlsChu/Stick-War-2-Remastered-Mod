package com.brockw.stickwar.engine.units
{
   ...
   public class Unit extends Entity
   {
      ...
      protected var _lastTurnCount:int = 0;
      
      protected var TURN_AGILITY:int = 5;
      ...
      protected var _createTime:int = 30;
      ...
      private var _healTimeRemaining:int = 0;
      ...
      private var _isRejoiningFormation:Boolean = false;
      
      private var garrisonHealRate:Number = 0;
      ...
      public function Unit(game:StickWar)
      {
         ...
         this.chaosHealRate = game.xml.xml.Chaos.healRate;
         this.garrisonHealRate = game.xml.xml.garrisonHealRate;
         this.nudgeFrame = game.frame;
         var glowColor:Number = 65535;
      ...
      public function firstInit() : void
      {
         ...
         this.healthBar.y = -otherHeight * 1.2;
         this.flyingHeight = 0;
         this._dz = 0;
         this.hasDefaultLoadout = false;
      }
      ...
      protected function updateCommon(game:StickWar) : void
      {
         ...
         if(Boolean(this.slowFramesRemaining))
         {
         ...
         else
         {
            ...
            if(Boolean(mouseIsOver) && this.isAlive())
            {
      ...
      protected function updateMotion(game:StickWar) : void
      {
         ...
         this._perspectiveScale = this._scale * (game.backScale + py / game.map.height * (game.frontScale - game.backScale));
         ...
         for each(w in this.team.enemyTeam.walls)
         {
            ...
            {
               px = w.px - BUF * this.team.direction;
            }
         }
         if(px < 0 + BUF)
         {
         ...
         if(u is Unit)
         {
            ...
            if(!u.mayWalkThrough && u.stunTimeLeft == 0 && u.isCollision(this,this.dx,this.dy))
            {
               ...
               if(hitAmount <= 1)
               {
                  if(px == u.px)
                  {
                     px = u.px + this.team.game.random.nextNumber() - 0.5;
                  }
                  if(px != u.px)
                  {
               ...
               hitWidth = (u.hitBoxWidth * u.perspectiveScale + this.hitBoxWidth * this.perspectiveScale) / 2;
               hitAmount = Math.abs(py - u.py) / hitWidth;
               if(hitAmount <= 1)
               {
                  if(py == u.py)
                  {
                     py = u.py + this.team.game.random.nextNumber() - 0.5;
                  }
                  if(py != u.py)
                  {
